...
class GameViewController: UIViewController {
    ...
    let wight: CGFloat = UIScreen.main.bounds.size.width //画面幅
    ...
    @IBAction func stop(){
        ...
        for i in 0..<3{
            score = score - abs(Int(wight/2 - positionX[i])) * 2 //スコアの計算
        }
        ...
        if (score > highScore1){ //ランキング1位の記録を更新したら
            ...
            defaults.set(highScore1, forKey: "score2") //"score2"というキーでhighscore1を保存
            defaults.set(highScore2, forKey: "score3") //"score3"というキーでhighscore2を保存
    ...
    @IBAction func retry(){
        ...
        positionX = [wight/2, wight/2, wight/2] //画像の位置を真ん中に戻す
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        positionX = [wight/2, wight/2, wight/2] //画像の位置を画面幅の中心にする
        self.start()
    ...
    @objc func up(){
        for i in 0..<3{
            //幅に来たら動かす向きを逆にする
            if(positionX[i] > wight || positionX[i] < 0){
                dx[i] = dx[i] * (-1)
            }
